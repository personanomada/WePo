...
import tempfile
import zipfile
from typing import Dict, List, Tuple, Optional, Iterable, Callable, Awaitable, AsyncGenerator
...
def _allow_echo(src: str, glossary: str) -> bool:
    ...
    if _URL_RE.search(s): return True
    if not re.search(r"[A-Za-zΑ-Ωα-ωÀ-ÿ]", s): return True
...
async def verify_provider(payload: Settings):
    ...
    if s.provider == "openai_compat":
        ...
        if not base: raise HTTPException(400, "OpenAI base_url missing")
        if "openai.com" in base and not key: raise HTTPException(400, "OpenAI API key missing")
        ...
        async with httpx.AsyncClient(timeout=30) as client:
            body = {"model": model, "temperature": temp, "response_format": {"type": "json_object"}, "messages": messages}
            ...
            if r.status_code == 400 and "response_format.type" in r.text and "json_schema" in r.text:
                json_schema = {"type": "json_schema", "json_schema": {"name": "verify_items", "schema": {"type": "object", "additionalProperties": False, "required": ["items"], "properties": {"items": {"type": "array", "items": {"type": "object", "additionalProperties": False, "required": ["key", "text"], "properties": {"key": {"type": "string"}, "text": {"type": "string"}}}}}}}}
        ...
        if r.status_code >= 400: raise HTTPException(400, f"HTTP {r.status_code}: {r.text[:200]}")
    ...
    if s.provider == "ollama":
        ...
        async with httpx.AsyncClient(timeout=15) as client:
            r = await client.get(f"{host}/api/tags")
        if r.status_code >= 400: raise HTTPException(400, f"Ollama HTTP {r.status_code}: {r.text[:200]}")
        try:
            tags = r.json().get("models", [])
            models = [m.get("model") or m.get("name") for m in tags if isinstance(m, dict)]
        except Exception:
...
async def analyze(po: UploadFile = File(...)):
    ...
    total, plurals, untranslated = len(catalog), sum(1 for e in catalog if e.msgid_plural), sum(1 for e in catalog if not e.translated())
    samples, audit = extract_samples_from_catalog(catalog), audit_po(catalog)
    summary_text = f"{total} entries. {plurals} plural blocks. {untranslated} untranslated. Issues found: {len(audit.get('issues', []))}."
    info = dict(headers=headers, counts=dict(entries=total, plurals=plurals, untranslated=untranslated), placeholderSamples=samples[:20], summaryText=summary_text)
    return AnalyzeOut(ok=True, info=info, audit=audit)

def _build_items_for_locale(catalog: polib.POFile, target_locale: str) -> Tuple[List[dict], Dict[str, Tuple[int, Optional[int]]]]:
    items: List[dict] = []
...
async def _translate_items_with_retries(provider, all_items: List[dict], source_lang: str, target_locale: str, batch_size: int, system_prompt: str, glossary: str) -> AsyncGenerator[Dict, None]:
    ...
    def _batches(values: Iterable[dict]) -> List[List[dict]]:
        return [list(values)[i: i + batch_size] for i in range(0, len(list(values)), batch_size)]

    for attempt in range(1, MAX_RETRIES + 2):
        if not remaining: break
        for batch in _batches(list(remaining.values())):
            try:
                result = await provider.translate_batch(batch=batch, source_lang=source_lang, target_locale=target_locale, system_prompt=system_prompt, glossary=glossary)
            except Exception as e:
            ...
            for k, v in result.items():
                ...
                if v == src_text and not _allow_echo(src_text, glossary):
                    echoed += 1
                    continue
                translated[k], newly = v, newly + 1
                del remaining[k]
            if newly > 0 or echoed > 0:
                yield {"type": "progress", "newly": newly, "echoed": echoed}
    
    if remaining:
        raise HTTPException(status_code=400, detail=f"Missing translation for {next(iter(remaining.keys()))} ({target_locale}). The provider did not return all items.")
    
    yield translated


def _fill_catalog_from_map(src_catalog: polib.POFile, out_catalog: polib.POFile, translated_map: Dict[str, str], target_locale: str):
    npl = nplurals_for_locale(target_locale)
    for idx, e in enumerate(src_catalog):
        ...
        if e.msgid_plural:
            ...
            for f in range(npl):
                ...
                if t is None: raise HTTPException(status_code=400, detail=f"Missing translation for {key} ({target_locale}). The provider did not return all plural forms.")
                try: validate_translation_placeholders(expected, t)
                except ValueError as ve: raise HTTPException(status_code=400, detail=f"Placeholder error in {key} ({target_locale}): {ve}")
                out_catalog[idx].msgstr_plural[f] = t
        else:
            key = f"{idx}"
            t = translated_map.get(key)
            if t is None: raise HTTPException(status_code=400, detail=f"Missing translation for {key} ({target_locale}).")
            try: validate_translation_placeholders(expected, t)
            except ValueError as ve: raise HTTPException(status_code=400, detail=f"Placeholder error in {key} ({target_locale}): {ve}")
            out_catalog[idx].msgstr = t

@app.post("/translate")
async def translate(po: UploadFile = File(...), locales: str = Form(...), sourceLang: str = Form("en"), settings_json: Optional[str] = Form(None)):
    _ensure_po_file(po)
    try: content = await po.read(); src_catalog = polib.pofile(content.decode("utf-8", errors="replace"))
    except Exception as e: raise HTTPException(status_code=400, detail=f"Invalid PO file: {e}")
    ...
    if not locales_list: raise HTTPException(status_code=400, detail="No target locales provided")
    out_per_locale: Dict[str, polib.POFile] = {}
    for loc in locales_list:
        items, _ = _build_items_for_locale(src_catalog, loc)
        translated_map = {}
        async for result in _translate_items_with_retries(provider=provider, all_items=items, source_lang=sourceLang, target_locale=loc, batch_size=s.batch_size, system_prompt=s.system_prompt, glossary=s.glossary):
            if result.get("type") != "progress": translated_map = result
        out = _clone_catalog_structure(src_catalog); out.metadata["Language"] = loc; out.metadata["Plural-Forms"] = get_plural_forms_header(loc)
        _fill_catalog_from_map(src_catalog, out, translated_map, loc); out_per_locale[loc] = out
    zip_bytes = _build_zip_bytes(out_per_locale)
    return StreamingResponse(io.BytesIO(zip_bytes), media_type="application/zip", headers={"Content-Disposition": 'attachment; filename="translations.zip"'})

@app.post("/translate/ndjson")
async def translate_ndjson(po: UploadFile = File(...), locales: str = Form(...), sourceLang: str = Form("en"), settings_json: Optional[str] = Form(None)):
    _ensure_po_file(po)
    try: content = await po.read(); src_catalog = polib.pofile(content.decode("utf-8", errors="replace"))
    except Exception as e: raise HTTPException(status_code=400, detail=f"Invalid PO file: {e}")
    s = EFFECTIVE_SETTINGS if not settings_json else Settings(**json.loads(settings_json)).normalized()
    ...
    async def generator():
        ...
        overall_done, overall_echoed = 0, 0
        try:
            for loc in locales_list:
                all_items_for_locale, _ = _build_items_for_locale(src_catalog, loc)
                translated_map = {}
                async for progress_or_result in _translate_items_with_retries(
                    provider=provider, all_items=all_items_for_locale, source_lang=sourceLang, 
                    target_locale=loc, batch_size=s.batch_size, 
                    system_prompt=s.system_prompt, glossary=s.glossary
                ):
                    if progress_or_result.get("type") == "progress":
                        overall_done += progress_or_result.get("newly", 0)
                        overall_echoed += progress_or_result.get("echoed", 0)
                        yield json.dumps({"type": "progress", "done": overall_done, "total": grand_total, "echoed": overall_echoed}) + "\n"
                    else:
                        translated_map = progress_or_result
                out = _clone_catalog_structure(src_catalog); out.metadata["Language"] = loc; out.metadata["Plural-Forms"] = get_plural_forms_header(loc)
                _fill_catalog_from_map(src_catalog, out, translated_map, loc); produced[loc] = out
